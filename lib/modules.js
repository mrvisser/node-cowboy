...
var init = module.exports.init = function(ctx, callback) {
    ctx.config().modules = ctx.config().modules || {};
    _npm = ctx.config().modules.npm || 'npm';

    _dir = ctx.config().modules.dir || '.';
...
var install = module.exports.install = function(uri, callback) {
    ...
    _exec(util.format('%s install "%s" --json', _npm, uri), {'cwd': _dir}, function(err, stdout) {
        ...
        if (!_.isArray(stdout)) {
            ...
            cowboy.logger.system().error({'stdout': stdout}, 'Module "%s" installed correctly, but npm output (stdout) was not a JSON array', uri);
            return callback(err);
        } else if (stdout.length === 0) {
            err = new Error('Expected npm install output to be an array of length > 0');
            cowboy.logger.system().error({'stdout': stdout}, 'Module "%s" installed correctly, but npm output (stdout) was not a JSON array with length > 0', uri);
            return callback(err);
        } else if (!stdout[0].name) {
            err = new Error('Expected npm install output module to have a module "name"');
            cowboy.logger.system().error({'stdout': stdout}, 'Module "%s" installed correctly, but npm output (stdout) was not a module that had a "name" attribute', uri);
            return callback(err);
        }
