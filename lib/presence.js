...
var broadcast = module.exports.broadcast = function(callback) {
    ...
    clearInterval(_intervalHandle);
    _intervalHandle = setInterval(internal.present, cowboy.data.get('hostname'), Date.now(), _interval);
    return internal.present(cowboy.data.get('hostname'), Date.now(), callback);
};
