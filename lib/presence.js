...
var init = module.exports.init = function(ctx, callback) {
    destroy(function() {
        ctx.config().presence = ctx.config().presence || {};
        _.defaults(ctx.config().presence, {'interval': 1000, 'timeout': 3000});
        _timeout = ctx.config().presence.timeout;
        _interval = ctx.config().presence.interval;
        return callback();
    });
};
...
var consume = module.exports.consume = function(callback) {
    internal.consume(function(err, hostMap) {
        if (err) {
            ...
            _hostMap = {};
            cowboy.logger.system().trace('Successfully consumed empty presence');
            return callback();
        }

        _hostMap = {};
        _.each(hostMap, function(lastPresent, host) {
            if (Date.now() - lastPresent < _timeout) {
                _hostMap[host] = lastPresent;
                cowboy.logger.system().trace('Consumed presence for host "%s" (Last Present: %dms ago)', host, Date.now() - lastPresent);
            } else {
                cowboy.logger.system().trace('Expired presence for host "%s" (Last Present: %dms ago)', host, Date.now() - lastPresent);
            }
        });

        cowboy.logger.system().trace({'hostMap': _hostMap}, 'Finished consuming presence');
