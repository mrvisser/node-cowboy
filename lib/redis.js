...
var init = module.exports.init = function(config, callback) {
    ...
    _.defaults(config.redis, {'host': 'localhost', 'port': 6379, 'index': 0});

    // Create the subscriber and publisher clients
    _createClient(config.redis, function(err, subClient) {
        if (err) {
            return callback(err);
        }

        cowboy.logger.system().debug(_.pick(config.redis, 'host', 'port', 'index'), 'Initialized subscriber redis client');

        _createClient(config.redis, function(err, pubClient) {
            if (err) {
                return callback(err);
            }

            cowboy.logger.system().debug(_.pick(config.redis, 'host', 'port', 'index'), 'Initialized publisher redis client');
...
var createChannel = module.exports.createChannel = function(name) {
    ...
    channel.send = function(data, callback) {
        cowboy.logger.system().trace({'data': data}, 'Sending message into channel');
...
/**
 * Create a listener that will only be invoked once. But will wait until the appropriate event for the appropriate channel
 * occurrs first before performing the invoke.
 *
 * This is necessary because the subscribe/unsubscribe events for redis are global, whereas we want to subscribe /
 * unsubscribe from individual channels. To accomplish this, we filter out all sub/unsub events that are not intended for
 * our channel, and only perform the event once when the correct event comes along.
 */
