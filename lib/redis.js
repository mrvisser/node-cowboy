...
var init = module.exports.init = function(host, port, index, password, callback) {
    _createClient(host, port, index, password, function(err, _subscriberClient) {
        ...
        _createClient(host, port, index, password, function(err, _persistentClient) {
            ...
            emitter.emit('init', host, port, index, password, subscriberClient, persistentClient);
            return callback();
        });
    });
};
 ...
 */
var request = module.exports.request = function(command, args, filters, handler) {
    cowboy.logger.system().trace({'command': command, 'args': args, 'filters': filters}, 'Init sending cowboy request');
    _generateMessageId(function(err, messageId) {
        ...
        subscriberClient.on('pmessage', function(pattern, channel, response) {
            emitter.emit('reply-receive', pattern, channel, response);
        ...
        emitter.emit('request-send', _getRequestChannel(), request);
...
var listenRequest = module.exports.listenRequest = function(handler) {
    ...
    subscriberClient.on('pmessage', function(pattern, channel, message) {
        ...
        emitter.emit('request-receive', pattern, channel, message);

        // Pass the request to the handler
        cowboy.logger.system().debug({'request': request}, 'Received request from cowboy');
        handler(request.filters, request.command, request.args, function(name, code, reply, published) {
            ...
            emitter.emit('reply-send', _getReplyChannel(request.messageId), response);
